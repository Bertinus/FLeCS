...
class CellPopulation(ABC, torch.nn.Module):
    def __init__(
        ...
        scale_factor_state_prior: float = 1.0,
    ):
        """
    ...
    def _delete_node_set(self, n_type: str) -> None:
        ...
        def truncate(x, n_set_to_be_del):
            return torch.cat([x[:, : n_set_to_be_del.idx_low], x[:, n_set_to_be_del.idx_high:]], dim=1)

        # Remove the corresponding state / production rates / decay rates
        self._state = truncate(self.state, node_set_to_be_del)
        self.production_rates = truncate(self.production_rates, node_set_to_be_del)
        self.decay_rates = truncate(self.decay_rates, node_set_to_be_del)
