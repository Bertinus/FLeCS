...
cebpe_shortest_paths = adata.uns["cebpe_ko_shortest_paths"]
# We subsample the paths to make sure they are of length n_bins
cebpe_shortest_paths = {k: v for k, v in cebpe_shortest_paths.items() if len(v) >= path_length}
cebpe_shortest_paths = {k: subsample_to_length(v, path_length) for k, v in cebpe_shortest_paths.items()}
# Initialize dataset
cebpe_dataset = Paul15Dataset(adata, cebpe_shortest_paths, path_length=path_length, option="late")

test_len = len(cebpe_dataset) - train_len - valid_len

train_dataset, valid_dataset, test_dataset = torch.utils.data.random_split(
    cebpe_dataset, [train_len, valid_len, test_len], generator=torch.Generator().manual_seed(0)
)
