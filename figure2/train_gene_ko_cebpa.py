...
cebpa_shortest_paths = adata.uns["cebpa_ko_shortest_paths"]
# We subsample the paths to make sure they are of length n_bins
cebpa_shortest_paths = {k: v for k, v in cebpa_shortest_paths.items() if len(v) >= path_length}
cebpa_shortest_paths = {k: subsample_to_length(v, path_length) for k, v in cebpa_shortest_paths.items()}
# Initialize dataset
cebpa_dataset = Paul15Dataset(adata, cebpa_shortest_paths, path_length=path_length, option="late")

test_len = len(cebpa_dataset) - train_len - valid_len

train_dataset, valid_dataset, test_dataset = torch.utils.data.random_split(
    cebpa_dataset, [train_len, valid_len, test_len], generator=torch.Generator().manual_seed(0)
)
