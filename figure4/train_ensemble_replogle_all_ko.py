...
all_kos = list(adata.obs["gene"].unique())
all_kos.remove('non-targeting')  # Handled in a separate script
...
for ko in all_kos:
    ...
    for seed in range(10):
        ...
        s_g2_ko_paths = adata.uns["s_g2_" + str(ko) + "_shortest_paths"]
        s_g2_ko_paths = {k: v[:-2] for k, v in s_g2_ko_paths.items()}

        unsorted_shortest_paths = {**adata.uns["g1_s_" + str(ko) + "_shortest_paths"], **s_g2_ko_paths,
                                   **adata.uns["g2_g1_" + str(ko) + "_shortest_paths"]}
        ...
        # Remove outgoing edges of KO gene
        ko_gene_idx = mycellpop["gene"].name.index(ko)
        outgoing_edges = (mycellpop['gene', 'regulates', 'gene'].edges[:, 0] == ko_gene_idx)
        mycellpop['gene', 'regulates', 'gene'].remove_edges(outgoing_edges)
        ...
        torch.save({
            ...
            **mycellpop["gene", "regulates", "gene"].state_dict()
            ...
            os.path.join(get_project_root(), "logs",
                         "trained_mycellpop_feb_15_1_ko_" + str(ko) + "_seed_" + str(seed) + ".pt")
        )
