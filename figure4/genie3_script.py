...
for env in list(adata.obs['gene'].unique()): 
    # ARGLU1 ASF1B MSN DNAJC3 MLLT6 STK38L HSPG2 CHPF TBX1 BUB1 KIF11 RCN1
    print(env)
    expr_data = np.array(adata[adata.obs['gene'] == env].X)
    if expr_data.shape[0] > 2000:
        expr_data = expr_data[np.random.choice(expr_data.shape[0], 2000, replace=False)]
    ...
    res = GENIE3(expr_data, 
            ...
            ntrees=100,
            nthreads=96)
    np.save(os.path.join(get_project_root(), "figure4", "processed", "genie3_res_" + env + ".npy"), res)
